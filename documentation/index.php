<?php include_once '_header.php' ?>

	<div class="hero-unit">
		<h1><i class="icon-book"></i> Phreeze Documentation</h1>
		<p>Documentation and code samples for the <a href="http://phreeze.com/">Phreeze Framework</a> for PHP.</p>
	</div>

	<h3>Getting Started</h3>
	
	<ul>
		<li><a href="introduction.php">Introduction to Phreeze</a></li>
		<li><a href="videos.php">Training Videos</a>
		<li><a href="installation.php">Installing Phreeze</a></li>
		<li><a href="builder.php">Phreeze Builder</a></li>
	</ul>
	
	<h3>Resources</h3>
	
	<ul>
	<li><a href="assets/cargo.sql.zip">Example MySQL Schema: cargo.sql.zip</a></li>
	<li><a href="assets/authexample.zip">Authentication Example App: authexample.zip</a></li>
	<li><a href="https://github.com/jasonhinkle/phreeze/issues">Phreeze Support Forum</a></li>
	</ul>
	
	<h3>The Basics</h3>
	
	<ul>
		<li><a href="configuration.php">Configuration Files</a></li>
		<li><a href="routes.php">Routes</a></li>
		<li><a href="controllers.php">Controllers</a></li>
		<li><a href="models.php">Models</a></li>
		<li><a href="reporters.php">Reporters</a></li>
		<li><a href="criteria.php">Criteria</a></li>
		<li><a href="views.php">Views</a></li>
		<li><a href="dispatcher.php">The Dispatcher</a></li>
	</ul>
	
	<h3>Advanced Topics</h3>
	
	<ul>
		<li><a href="debug.php">Debugging Phreeze</a></li>
		<li><a href="unittests.php">Unit Testing</a></li>
		<li><a href="docs.php">Generating API Documentation</a></li>
	</ul>
	
	<h3>Phreeze Inner Workings</h3>
	
	<ul>
		<li><a href="http://phreeze.com/docs/">API Documentation</a>
		<li><a href="maps.php">Field and Key Maps</a></li>
		<li><a href="querybuilder.php">Query Builder</a></li>
		<li><a href="renderengine.php">View Render Engines</a></li>
	</ul>

<?php include_once '_footer.php' ?>